...
def get_entity_position_embedding(tokenizer, entity_marker_type, special_tokens, input_ids):
    ...
    if entity_marker_type == "typed_entity_marker":
        ...
        for s_emb, o_emb, ids in zip(subj_emb, obj_emb, input_ids):
            ...
            temp_smark_num = ids[s_emb[0] + 2 : s_emb[1]].tolist().index(sub_marker_id) + 1
            temp_omark_num = ids[o_emb[0] + 2 : o_emb[1]].tolist().index(obj_marker_id) + 1
            temp_s_embeddings[s_emb[0] + 2 + temp_smark_num : s_emb[1]] = [1] * (s_emb[1] - (s_emb[0] + 2 + temp_smark_num))
            temp_o_embeddings[o_emb[0] + 2 + temp_omark_num : o_emb[1]] = [1] * (o_emb[1] - (o_emb[0] + 2 + temp_omark_num))
            subj_embeddings.append(temp_s_embeddings)
            obj_embeddings.append(temp_o_embeddings)
