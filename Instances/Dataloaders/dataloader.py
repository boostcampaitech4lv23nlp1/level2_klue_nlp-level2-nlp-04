...
class Dataloader(pl.LightningDataModule):
    def __init__(self, conf):
        ...
        self.data_collator = transformers.DataCollatorWithPadding(
            self.tokenizer
        )  # 다이나믹 패딩 유튜브 -> 잘안되는거 같음 (train 237로만 잘르고 dev 241)
    ...
    def tokenizing(self, dataframe):
        ...
        for e01, e02 in zip(dataframe["subject_entity"], dataframe["object_entity"]):
            temp = ""
            temp = (
                e01 + self.tokenizer.sep_token + e02
        ...
        tokenized_sentences = self.tokenizer(
            ...
            list(
                dataframe["sentence"]
            ),  # [CLS](sub[SEP]object_entity)[SEP]sentence[SEP] 형태, sentence 부분이 1로, 나머지는 0으로 세그먼트 임베딩이 될 것 같습니다
            return_tensors="pt",
            padding=True,
            truncation=True,
    ...
    def preprocessing(
        self, dataframe, labels_exist=True
    ):  # 전체 전처리 과정을 모두 거쳐서 dataset input 형태로 구성할 수 있도록 하고 predict일 땐 빈 배열 반환
    ...
    def setup(self, stage="fit"):  # train, dev, test는 모두 동일한 전처리 과정을 거칩니다
        if stage == "fit":
            ...
            train_data = total_data.sample(
                frac=self.train_ratio
            )  # csv 파일을 불러서 train과 dev로 나눕니다, 기본 baseline의 load_data 과정
